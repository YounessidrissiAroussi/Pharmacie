...
function Caisse() {
  ...
  const AddProduit = (e) =>{
    e.preventDefault()
      const FindItem = produit.find(x=>x.codebarre === barcode.trim())
      if(FindItem){
        FindItem.qte = 1 
  ...
  const mapping = findProduct.map((item,index)=>(
      <tr key={item.id}>
        <td>{index + 1}</td>
        <td>{PatientItem}</td>
  ...
  <div className="form-group mb-2">
    ...
    <input type="text" className="form-contro" id="staticEmail2" value={barcode} placeholder='Code Barre' onChange={e=>setBarCode(e.target.value)} />
  </div>
  <div className="form-group mx-2 mb-2">
    <label htmlFor="staticEmail2" className="sr-only">Patient</label>
    <select id="Patient_id" className="form-select"  name='Patient_id' disabled={PatientItem ? true : false} onChange={e=>{setPatientItem(e.target.value)}}>
...
<table className="table table-fixed">
        ...
        {mapping}
          ...
          <tr>
            ...
            <td></td>
            <td></td>
            <td><button type="submit" className="btn btn-warning px-5 py-1" onClick={Calculer} ><i className="fa fa-check"></i></button></td>
            <td></td>
          </tr>
